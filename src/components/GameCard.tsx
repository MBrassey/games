...
export default function GameCard({ game }: { game: GameEntry }) {
  ...
  // Cursor spotlight: writes --mx/--my CSS custom properties on the link
  // element as the pointer moves. The ::before pseudo (see .card-spotlight
  // in globals.css) reads them for a radial-gradient follow effect.
  // Pure CSS render — React doesn't re-render per frame, just nudges vars.
  ...
  return (
    <Link
      ...
      className={`card-spotlight group relative block overflow-hidden border border-eldritch-deep/60 bg-void-1/70 p-5 transition-all hover:border-eldritch-purple hover:panel-glow ${disabled ? "is-disabled" : ""}`}
      style={
        {
          boxShadow: `inset 0 0 0 1px rgba(138,79,255,0.08)`,
          "--game-accent": game.accentColor,
        } as React.CSSProperties
      }
    >
      {/* corner brackets */}
      <span className="pointer-events-none absolute left-0 top-0 h-3 w-3 border-l border-t z-10" style={{ borderColor: game.accentColor }} />
      <span className="pointer-events-none absolute right-0 top-0 h-3 w-3 border-r border-t z-10" style={{ borderColor: game.accentColor }} />
      <span className="pointer-events-none absolute left-0 bottom-0 h-3 w-3 border-l border-b z-10" style={{ borderColor: game.accentColor }} />
      <span className="pointer-events-none absolute right-0 bottom-0 h-3 w-3 border-r border-b z-10" style={{ borderColor: game.accentColor }} />
